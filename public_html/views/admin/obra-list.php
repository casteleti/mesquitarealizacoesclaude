...
<div class="panel table-wrap">
    ...
    <?php else: ?>
        <table>
            <thead><tr><th>Título</th><th>Setor</th><th>Status</th><th>Destaque</th><th>Ações</th></tr></thead>
            <tbody>
                <?php foreach ($items as $item): ?>
                    <tr>
                        <td data-label="Título"><?= e($item['title']) ?></td>
                        <td data-label="Setor"><?= e($item['setor_nome'] ?? '—') ?></td>
                        <td data-label="Status"><span class="badge <?= $item['ativo'] ? 'badge-success' : 'badge-muted' ?>"><?= $item['ativo'] ? 'Publicado' : 'Oculto' ?></span></td>
                        <td data-label="Destaque"><?= $item['destaque'] ? '<span class="badge badge-gold">Sim</span>' : '<span class="badge badge-muted">Não</span>' ?></td>
                        <td data-label="Ações" class="actions">
                            <a href="<?= e(url('admin/obras/' . $item['id'] . '/edit')) ?>">Editar</a>
                            <form method="post" action="<?= e(url('admin/obras/' . $item['id'] . '/delete')) ?>" data-confirm="Remover esta obra?">
